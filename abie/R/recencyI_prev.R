...
#' @details Implements assay-based incidence estimation through cross-sectional prevalance and recency of infection tests. Function parameters must be specified to include assay test characteristics and survey results as proportions. Confidence intervals are computed via Delta method approximation, except when Boot=TRUE is specified, in which case confidence intervals are generated by empirical bootstrap resampling. Inputs must be in appropriate ranges for appropriate untis. Extreme input values may make calculation impossible, and if entered will elicit error notices.
#'
#' @examples
...
#'PrevR=c(0.10,0.13,0.12), RSE_PrevR=c(0.094,0.095,0.05), BS_Count=10000,
#'Boot=FALSE, BMest="MDRI.FRR.indep", MDRI=200, RSE_MDRI=0.05,
#'FRR=c(0.01,0.009,0.02), RSE_FRR=0.2, BigT=730)
#' @export
...
#' @param Covar_HR Covariance of probability of being postive and being categorized recent from survey (or as a vector for multiple surveys)
#' @details Implements assay-based incidence estimation through cross-sectional prevalance and recency of infection tests. Function parameters must be specified to include assay test characteristics and survey results as counts. Confidence intervals are computed via Delta method approximation, except when Boot=TRUE is specified, in which case confidence intervals are generated by empirical bootstrap resampling. Inputs must be in appropriate ranges for appropriate untis. Extreme input values may make calculation impossible, and if entered will elicit error notices.
#' @return Incidence estimate, confidence interval, relative standard error. If multiple surveys are entered, function returns said results, as well as estimates of indcidence differences, confidence intervals of differences, differences in relative standard error, and p-values testing the hypothesis that the indcidence measures are different.
#' @examples
#' incBYcounts(N = c(5000) ,N_H = 1000, N_testR = 1000, N_R = 70, BSoot = FALSE,
#' BMest = "MDRI.FRR.indep", MDRI = 200, RSE_MDRI = 0.05, FRR = 0.01, RSE_FRR = 0.2, BigT = 730)
#'
#' incBYcounts(N = c(4000,4000,4050) ,N_H = c(1010,1000,900), N_testR = c(1000,1000,880),
#' N_R = c(60,70,50), Boot=TRUE, BMest="same.test", MDRI = 210, RSE_MDRI = 0.05,
#' FRR = 0.005, RSE_FRR = 0.19, BigT = 700)
#'
#' incBYcounts(N = c(4000,4000,4000) ,N_H = c(1050,1090), N_testR = c(1000,1000),
#' N_R = c(60,67), Boot=FALSE, BMest="FRR.indep", MDRI = 220, RSE_MDRI = 0.05,
#' FRR = c(0.005,0.005,0.01), RSE_FRR = 0.19, BigT = 610)
#' @export
