...
#'
#' @details Type vignette("Inference",package="abie") into console for more details on this function.
#'
#' @examples
...
prevBYcounts <- function(N, N_H, N_testR, N_R, DE_H = 1, DE_R = 1) {
  ...
  stopifnot(no_s == length(N_H) & no_s == length(N_R) & no_s == length(N_testR) & no_s == length(DE_H) & no_s == length(DE_R))
  PrevH <- N_H/N
  PrevR <- N_R/N_testR
...
BS_SURVEY_ESTS <- function(prevH, prevR, mdri, frr, bs_count, bs_var_prevH, bs_var_prevR, bs_var_mdri, bs_var_frr, covar_HR) {
  Mu <- c(prevH, prevR, mdri, frr)
  sigma <- matrix(c(bs_var_prevH, covar_HR, 0, 0, covar_HR, bs_var_prevR, 0, 0, 0, 0, bs_var_mdri, 0, 0, 0, 0, bs_var_frr),
                  nrow = 4, ncol = 4)
  # bs_var_prevH, bs_var_prevR, and so on are...?empirical, observed variance of variable? returns bootstraps of prevH,
  # prevR, mdri, frr
...
DM_VAR_deltaI <- function(BMest, fot_prevH1, fot_prevR1, fot_mdri1, fot_frr1, fot_prevH2, fot_prevR2, fot_mdri2, fot_frr2,
                          dm_var_prevH1, dm_var_prevR1, dm_var_mdri1, dm_var_frr1, dm_var_prevH2, dm_var_prevR2, dm_var_mdri2, dm_var_frr2) {
  if (BMest == "same.test") {
    DM_Var_deltaI <- ((fot_prevH1^2) * dm_var_prevH1) + ((fot_prevH2^2) * dm_var_prevH2) + ((fot_prevR1^2) * dm_var_prevR1) +
      ((fot_prevR2^2) * dm_var_prevR2) + ((fot_mdri1 - fot_mdri2)^2 * dm_var_mdri1) + ((fot_frr1 - fot_frr2)^2 * dm_var_frr1)
  } else if (BMest == "FRR.indep") {
    DM_Var_deltaI <- ((fot_prevH1^2) * dm_var_prevH1) + ((fot_prevH2^2) * dm_var_prevH2) + ((fot_prevR1^2) * dm_var_prevR1) +
      ((fot_prevR2^2) * dm_var_prevR2) + ((fot_mdri1 - fot_mdri2)^2 * dm_var_mdri1) + ((fot_frr1^2) * dm_var_frr1) +
      ((fot_frr2^2) * dm_var_frr2)
  } else if (BMest == "MDRI.FRR.indep") {
    DM_Var_deltaI <- ((fot_prevH1^2) * dm_var_prevH1) + ((fot_prevH2^2) * dm_var_prevH2) + ((fot_prevR1^2) * dm_var_prevR1) +
      ((fot_prevR2^2) * dm_var_prevR2) + ((fot_mdri1^2) * dm_var_mdri1) + ((fot_mdri2^2) * dm_var_mdri2) + ((fot_frr1^2) *
...
DM_VAR_deltaI.infSS <- function(BMest, fot_mdri1, fot_frr1, fot_mdri2, fot_frr2, dm_var_mdri1, dm_var_frr1, dm_var_mdri2,
                                dm_var_frr2) {
  if (BMest == "same.test") {
    DM_Var_deltaI <- ((fot_mdri1 - fot_mdri2)^2 * dm_var_mdri1) + ((fot_frr1 - fot_frr2)^2 * dm_var_frr1)
  } else if (BMest == "FRR.indep") {
    DM_Var_deltaI <- ((fot_mdri1 - fot_mdri2)^2 * dm_var_mdri1) + ((fot_frr1^2) * dm_var_frr1) + ((fot_frr2^2) * dm_var_frr2)
  } else if (BMest == "MDRI.FRR.indep") {
    DM_Var_deltaI <- ((fot_mdri1^2) * dm_var_mdri1) + ((fot_mdri2^2) * dm_var_mdri2) + ((fot_frr1^2) * dm_var_frr1) +
      ((fot_frr2^2) * dm_var_frr2)
  } else {
    stop("specify BMest")
  }
...
# example to get function working: probs<-prevBYcounts (N=c(5000,5000), N_H=c(1000,1000), N_testR=c(1000,1000),
# N_R=c(100,70)) probs PrevH = probs[1,1] RSE_PrevH = probs[1,3] PrevR = probs[1,2] RSE_PrevR = probs[1,4] BS_Count =
# 10000 Boot = FALSE BMest = 'MDRI.FRR.indep' MDRI = 200 RSE_MDRI = 0.05 FRR = 0.01 RSE_FRR = 0.2 BigT = 730 Covar_HR=0
# alpha=0.05 recencyI(PrevH=probs[,1], RSE_PrevH=probs[,3], PrevR=probs[,2], RSE_PrevR=probs[,4], Boot = FALSE, BS_Count
# = 10000, alpha = 0.05, BMest = 'MDRI.FRR.indep', MDRI=MDRI, RSE_MDRI=RSE_MDRI, FRR=FRR, RSE_FRR=RSE_FRR, BigT =
# c(730,720), Covar_HR = 0)
...
recencyI <- function(PrevH, RSE_PrevH, PrevR, RSE_PrevR, Boot = FALSE, BS_Count = 10000, alpha = 0.05, BMest = "same.test",
                     MDRI, RSE_MDRI, FRR, RSE_FRR, BigT = 730, Covar_HR = 0) {
  ...
  stopifnot(no_s == length(PrevR) & no_s == length(RSE_PrevH) & no_s == length(PrevR) & no_s == length(MDRI) & no_s ==
              length(RSE_MDRI) & no_s == length(FRR) & no_s == length(RSE_FRR) & no_s == length(Covar_HR))
  ...
  # now each element in deltaI_Est_Mat is the difference in I for each survey the ij element is I[i]-I[j].


  deltaI_Est_Vec <- as.vector(deltaI_Est_Mat)
  # makes above matrix into a vector

  # this section defines the empirical variance of each object to be bootsrapped for the mvtrnorm function this section
  # was set up this way because originally there was the bootstrap smoother, which added variance from both sources, so
  # here I depending on whethere Boot=TRUE or not, I make the variance components their values under the variance scheme, or
  # 0. I leave this section intact in case Alex decides he wants to do smoothing after all
  if (Boot == TRUE) {
  ...
  if (Boot == TRUE) {
    ...
    for (i in 1:no_s) {
      BS_RootEstMat[, (i * 4 - 3):(i * 4)] <- BS_SURVEY_ESTS(prevH = PrevH[i], prevR = PrevR[i], mdri = MDRI[i], frr = FRR[i],
                                                             bs_count = BS_Count, bs_var_prevH = BS_Var_PrevH[i], bs_var_prevR = BS_Var_PrevR[i], bs_var_mdri = BS_Var_MDRI[i],
                                                             bs_var_frr = BS_Var_FRR[i], covar_HR = Covar_HR[i])
    }
    # returns bootstraps of prevH, prevR, mdri, frr as columns, one for each survey, so if survey#=3, then first four
    # columns are for prevH, prevR, mdri, frr as columns, then next four are for those varaibles from second survey, and so
    ...
    # calculates incidence (& Var) estimates based on bootstrapped samples of input parameters
    for (i in 1:no_s) {
      I_BSVec <- I_EST(prevH = BS_RootEstMat[, (i * 4 - 3)], prevR = BS_RootEstMat[, (i * 4 - 2)], mdri = BS_RootEstMat[,
                                                                                                                        (i * 4 - 1)], frr = BS_RootEstMat[, (i * 4)], bigt = BigT[i])
      I_BSMat[, i] <- I_BSVec
    ...
    # above makes empty matrix of nrow=#BS samples, ncol=#surveys squared, and inputs to each the difference for each BS
    ...
    for (i in c(1:no_s)) {
      ...
      # DM_FirstOrderTerms gives fot_prevH, fot_prevR, fot_mdri, fot_frr, so FOT for each prev. survey input and each row of
      # fot_Mat has the FOT for prev. survey i.
      DM_Var_I[i] <- (fot_Mat[i, 1]^2) * DM_Var_PrevH[i] + (fot_Mat[i, 2]^2) * DM_Var_PrevR[i] + (fot_Mat[i, 3]^2) *
        DM_Var_MDRI[i] + (fot_Mat[i, 4]^2) * DM_Var_FRR[i]
    ...
    for (i in c(1:no_s)) {
      for (j in c(1:no_s)) {
        ...
        DM_Var_deltaI[i * no_s - (no_s - j)] <- DM_VAR_deltaI(BMest = BMest, fot_prevH1 = fot_Mat[i, 1], fot_prevH2 = fot_Mat[j,
                                                                                                                              1], fot_prevR1 = fot_Mat[i, 2], fot_prevR2 = fot_Mat[j, 2], fot_mdri1 = fot_Mat[i, 3], fot_mdri2 = fot_Mat[j,
                                                                                                                                                                                                                                         3], fot_frr1 = fot_Mat[i, 4], fot_frr2 = fot_Mat[j, 4], dm_var_prevH1 = DM_Var_PrevH[i], dm_var_prevH2 = DM_Var_PrevH[j],
                                                              dm_var_prevR1 = DM_Var_PrevR[i], dm_var_prevR2 = DM_Var_PrevR[j], dm_var_mdri1 = DM_Var_MDRI[i], dm_var_mdri2 = DM_Var_MDRI[j],
                                                              dm_var_frr1 = DM_Var_FRR[i], dm_var_frr2 = DM_Var_FRR[j])

        DM_Var_deltaI.infSS[i * no_s - (no_s - j)] <- DM_VAR_deltaI.infSS(BMest = BMest, fot_mdri1 = fot_Mat[i,
                                                                                                             3], fot_frr1 = fot_Mat[i, 4], fot_mdri2 = fot_Mat[j, 3], fot_frr2 = fot_Mat[j, 4], dm_var_mdri1 = DM_Var_MDRI[i],
                                                                          dm_var_frr1 = DM_Var_FRR[i], dm_var_mdri2 = DM_Var_MDRI[j], dm_var_frr2 = DM_Var_FRR[j])
      }
    }
    # creates a vector of length (number of surveys)^2, each term is delta method variance applied to survey 1 & 1, 1&2,
    # 2&1, and 2&2.
  ...
  # this next function originally took the bootstrap matrix, SD via delta method, and I estimates, and returned the spread
  # version of those variables. I rewrote so it only deals with pure empirical BS, or Delta method. Now the method takes
  ...
  # above returns CIs for each element in deltaI_Est_Vec, which is a four-tuple vector for 2 surveys

  # Below gives p-value of tests of differences, and if Boot = FALSE, p-values of test of differences when SS=infinity gives 4
  # p-values for 2 surveys (redundant). Later this will be reduced down for outputing results.
  p_value <- pnorm((-abs(deltaI_Est_Vec)/SD_deltaI), mean = 0, sd = 1) * 2
  if (Boot == F) {
    p_value.infSS <- pnorm((-abs(deltaI_Est_Vec)/sqrt(DM_Var_deltaI.infSS)), mean = 0, sd = 1) * 2
  }
  ...
  # this entire section below was to put blank values in the multiple survey output, but much of that display has been
  ...
  # puts empty spaces in each place of vectors needed in calculations. as above, since much of this section is being
  # removed, the code here could be streamlined more.
  for (i in c(1:no_s)) {
  ...
  # section gives warnings if some confidence intervals exceed 0 bound for incidence
  ...
  # Now structure how the output should look first, if there's only one survey entered:
  if (length(I_Est) == 1) {
    ARI <- 1 - exp(-as.numeric(out_I_Est))
    ARI.CI.low <- 1 - exp(-as.numeric(out_CI_I_lo))
    ARI.CI.up <- 1 - exp(-as.numeric(out_CI_I_up))
    ARI.list <- round(data.frame(ARI = ARI, ARI.CI.low = ARI.CI.low, ARI.CI.up = ARI.CI.up), digits = 4)
    if (Boot == FALSE) {
      output <- list(Incidence.Statistics = data.frame(Incidence = out_I_Est, `CI low` = out_CI_I_lo, `CI up` = out_CI_I_up,
                                                       RSE = out_RSE_I, RSE.Inf.SS = out_RSE_I.infSS), Annual.Risk.of.Infection = ARI.list, MDRI.CI = MDRI.CI,
                     FRR.CI = FRR.CI)
    } else {
      output <- list(Incidence.Statistics = data.frame(Incidence = out_I_Est, `CI low` = out_CI_I_lo, `CI up` = out_CI_I_up,
                                                       RSE = out_RSE_I), Annual.Risk.of.Infection = ARI.list, MDRI.CI = MDRI.CI, FRR.CI = FRR.CI)
    }
  } else if (Boot == FALSE) {
    Incidence.Statistics = data.frame(survey = survey_no, Incidence = out_I_Est, `CI low` = out_CI_I_lo, `CI up` = out_CI_I_up,
                                      RSE = out_RSE_I, RSE.Inf.SS = out_RSE_I.infSS)
    Incidence.Difference.Statistics = data.frame(compare = delta_code, Diff = out_deltaI_Est, `CI Diff low` = out_CI_deltaI_Mat[,
                                                                                                                                1], `CI Diff up` = out_CI_deltaI_Mat[, 2], `RSE Diff` = out_RSE_deltaI, `RSE Diff Inf.SS` = out_RSE.deltaI.infSS,
    ...
    Incidence.Difference.Statistics = data.frame(compare = delta_code, Diff = out_deltaI_Est, `CI Diff low` = out_CI_deltaI_Mat[,
                                                                                                                                1], `CI Diff up` = out_CI_deltaI_Mat[, 2], `RSE Diff` = out_RSE_deltaI, `p-value` = out_p_value)
    Incidence.Statistics = data.frame(survey = survey_no, Incidence = out_I_Est, `CI low` = out_CI_I_lo, `CI up` = out_CI_I_up,
                                      RSE = out_RSE_I)
    Incidence.Statistics = Incidence.Statistics[which(Incidence.Statistics[, 1] != ""), ]
    Incidence.Difference.Statistics = Incidence.Difference.Statistics[which(!is.na(Incidence.Difference.Statistics[,
...
#'
#' @export
incBYcounts <- function(N, N_H, N_testR, N_R, DE_H = 1, DE_R = 1, BS_Count = 10000, Boot = FALSE, alpha = 0.05, BMest = "same.test",
                        MDRI, RSE_MDRI, FRR, RSE_FRR, BigT = 730, Covar_HR = 0) {
  ...
  recencyI(BS_Count = BS_Count, Boot = Boot, alpha = alpha, BMest = BMest, PrevH = counts.to.prev$PrevH, RSE_PrevH = counts.to.prev$RSE_PrevH,
           PrevR = counts.to.prev$PrevR, RSE_PrevR = counts.to.prev$RSE_PrevR, MDRI = MDRI, RSE_MDRI = RSE_MDRI, FRR = FRR,
           RSE_FRR = RSE_FRR, BigT = BigT, Covar_HR = Covar_HR)
}
...
#
# ################### == Call - DM only ==######################################################
 recencyI(BS_Count = 10000, Boot = FALSE, BMest = "same.test", PrevH = probs[, 1], RSE_PrevH = probs[, 3],
          PrevR = probs[,2], RSE_PrevR = probs[, 4], MDRI = 200, RSE_MDRI = 0.05, FRR = 0.01, RSE_FRR = 0.2, BigT = 730)
#
# # (single survey)
 recencyI(BS_Count = 10000, Boot = FALSE, BMest = "same.test", PrevH = probs[1, 1],
