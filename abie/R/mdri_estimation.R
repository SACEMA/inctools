...
# ADD OPTION TO GET FULL LIST OF MDRIs from the bootstrapping procedure or the shape of the distribution or something
  ...
  switch(as.character(recency_rule), binary_data = {
    ...
    for (i in 1:n_recvars) {
      if (recency_params[2 * i] == 0) {
        data$recencytemp <- ifelse(data[, 2 + i] < recency_params[2 * i - 1], 1, 0)
      }
  ...
  switch(as.character(functional_form), cloglog_linear = {
    ...
    while (!fitted) {
      model <- glm2::glm2(formula = (1 - recency_status) ~ 1 + I(log(time_since_eddi)), family = binomial(link = "cloglog"),
                          data = data, control = glm.control(epsilon = tolerance, maxit = maxit, trace = FALSE))
    ...
    while (!fitted) {
      model <- glm2::glm2(formula = recency_status ~ 1 + I(time_since_eddi) + I(time_since_eddi^2) + I(time_since_eddi^3),
                          family = binomial(link = "logit"), data = data, control = glm.control(epsilon = tolerance, maxit = maxit,
                                                                                                trace = FALSE))
      if (class(model)[1] == "try-error") {
        tolerance <- tolerance * 10
      } else {
...
integrate_for_mdri <- function(parameters = parameters, recency_cutoff_time = recency_cutoff_time, functional_form = functional_form,
                               tolerance = tolerance_integral, maxit = maxit_integral) {
  ...
  switch(as.character(functional_form), cloglog_linear = {
    ...
    if (class(answer) == "try-error") {
      cat("try-error", "\n")
      answer <- pracma::romberg(f = functional_form_clogloglinear, a = 0, b = recency_cutoff_time, parameters = parameters,
                                tol = tolerance, maxit = maxit)$value
    }
  }, logit_cubic = {
    answer <- try(cubature::adaptIntegrate(f = functional_form_logitcubic, lowerLimit = 0, upperLimit = recency_cutoff_time,
                                           parameters = parameters, tol = tolerance, fDim = 1, maxEval = 0, absError = 0, doChecking = FALSE)$integral)
    if (class(answer) == "try-error") {
      cat("try-error", "\n")
      answer <- pracma::romberg(f = functional_form_logitcubic, a = 0, b = recency_cutoff_time, parameters = parameters,
                                tol = tolerance, maxit = maxit)$value
    }
  })
  return(answer)
...
plot_probability <- function(functional_form = functional_form, parameters = parameters, mdri = mdri, inclusion_time_threshold = inclusion_time_threshold,
  ...
  switch(as.character(functional_form), cloglog_linear = {
    ...
    plotdata <- data.frame(plot_time, 1/(1 + exp(-(parameters[1] + parameters[2] * plot_time + parameters[3] * plot_time^2 +
