#This script was written by Petra Baumler as a test for different ways to make Alex's idea
#for hybrid delta-method/bootstrap confidence intervals faster.
#We will not use this in the final package.
...
for (i in c(1, 2)){
  ...
  sigma <- matrix( c(BS_Var_PrevH[i],  Covar_HR[i],     0,           0,
                     Covar_HR[i],      BS_Var_PrevR[i], 0,           0,
                     0,                0,               BS_Var_MDRI, 0,
                     0,                0,               0,           BS_Var_FRR),
                   nrow=4, ncol=4)
...
DM_Var_I      <- (fot_PrevH^2)*DM_Var_PrevH + (fot_PrevR^2)*DM_Var_PrevR +
                 (fot_MDRI^2) *DM_Var_MDRI   + (fot_FRR^2) *DM_Var_FRR
DM_SD_I <- sqrt(DM_Var_I)
DM_Var_deltaI <- sum((fot_PrevH^2)*DM_Var_PrevH)         + sum((fot_PrevR^2)*DM_Var_PrevR) +
                     (fot_MDRI[1]-fot_MDRI[2])^2*DM_Var_MDRI + (fot_FRR[1]-fot_FRR[2])^2*DM_Var_FRR
...
########################################################################################################
# I_spread <- function(I_BSVec, DM_Var_I) {
#    I_BSVec_sort<-sort(I_BSVec, decreasing=FALSE)
#
#   AUC_i <- function(x){
#      AUC <- vector(length=BS_Count)
#
#      for (i in c(1:BS_Count)) {
#         if((I_BSVec_sort[i]>(I_BSVec_sort[x]-4*sqrt(DM_Var_I))) & (I_BSVec_sort[i]<(I_BSVec_sort[x]+4*sqrt(DM_Var_I)))){
#              AUC[i] <- pnorm(I_BSVec_sort[x],   mean=I_BSVec_sort[i], sd=sqrt(DM_Var_I)) -
#                        pnorm(I_BSVec_sort[x-1], mean=I_BSVec_sort[i], sd=sqrt(DM_Var_I))}
#         else {AUC[i] <- NA}}
#      return(sum(AUC, na.rm=TRUE))}
#
#   distvec<-vector(length=BS_Count-1)
#   for (x in c(2:BS_Count)) {distvec[x] <- AUC_i(x)}
#   I_spreadVec <- distvec/sum(distvec)
#
#   CD <- vector()
#   for (i in c(1:(BS_Count-1))) {CD[i]<-sum(I_spreadVec[c(1:i)])}
#   xi_lo  <- findInterval(0.025,CD)
#   xi_up  <- findInterval(0.975, CD)
#   CIlo_I <- I_BSVec_sort[xi_lo]
#   CIup_I <- I_BSVec_sort[(xi_up+1)]
#   output<-data.frame("CIlo_Ispread"=CIlo_I, "CIup_Ispread"=CIup_I)
#     return (output)}
...
I_spread2 <- function() {
     ...
     for (i in c(1:no_s)) {
        SpreadMat[,i] <- seq(from=(I_est[i]-5*sqrt(Var_I[i])),
                             to=(I_est[i]+5*sqrt(Var_I[i])),
                             length.out=BSDM_spread)}
...
I_spread3 <- function() {
    ...
    p_seq <- seq(from=0.0001, to=0.9999, length.out=BSDM_spread)
...
I_spread2.2 <- function() {
  ...
  for (i in c(1:no_s)){
    ...
    auc <- vector(length=(BSDM_spread-1))
     for (j in c(1:(BSDM_spread-1))) {
         ...
         auc[j]<- sum((pnorm(spread_seq[j+1], mean=i_bsvec_use, sd=dm_sd))-(pnorm(spread_seq[j], mean=i_bsvec_use, sd=dm_sd)))}
    AUC <- auc/sum(auc)
    AUCMat[,i] <- AUC}
  ...
  for (i in c(1:no_s)) {
    AUC <- AUCMat[,i]
    CD <- vector ()
    for(j in c(1:(BSDM_spread-1))) {CD[j]<-sum(AUC[1:j])}
    xi_lo <- findInterval(0.025, CD)
    xi_up <- findInterval(0.975, CD)
    CIlo_I[i] <- SpreadMat[xi_lo,i]
    CIup_I[i] <- SpreadMat[(xi_up+1),i]}
...
##################################################################################################
# I_diag <- function(no_int, I_est, Var_I, I_BSVec, DM_Var_I)
# {
# no_int <- no_int
# kk     <- no_int-2
#
#   delta_i <- (8*sd(I_BSVec))/kk
#   ii      <- seq(from=(mean(I_BSVec)-4*sd(I_BSVec)-delta_i), to=(mean(I_BSVec)+4*sd(I_BSVec)), by=delta_i)
#   CD_I_BS <- ecdf(I_BSVec)
#   AUC_I_BS<- CD_I_BS(ii+delta_i) - CD_I_BS(ii)
#
#   delta_i <- (8*sqrt(DM_Var_I))/kk
#   ii      <- seq(from=(-4*sqrt(DM_Var_I)-delta_i), to=(4*sqrt(DM_Var_I)), by=delta_i)
#   AUC_I_DM<-pnorm((ii+delta_i),mean=0, sd=sqrt(DM_Var_I))-pnorm((ii),mean=0, sd=sqrt(DM_Var_I))
#
#   X      <- matrix(rep(AUC_I_BS, times=no_int), ncol=no_int, nrow=no_int)
#   Y      <- matrix(rep(AUC_I_DM, times=no_int),nrow=no_int, ncol=no_int)
#   AUC_3d <- t(X)*Y
#
#   M_ur <- sum(diag(AUC_3d))/2+sum(diag(AUC_3d[,-1]))/2
#   M_ll <- sum(diag(AUC_3d))/2+sum(diag(AUC_3d[-1,]))/2
#   C_ur <- sum(diag(AUC_3d[,-(1:kk)]))/2+(AUC_3d[1,no_int])/2
#   C_ll <- sum(diag(AUC_3d[-(1:kk),]))/2+(AUC_3d[no_int,1])/2
#   sumvec_ur <- vector(length=(kk-1))
#   for (i in c(1:(kk-1))){sumvec_ur[i] <- sum(diag(AUC_3d[,-(1:i)]))/2 + sum(diag(AUC_3d[,-(1:(i+1))]))/2}
#   sumvec_ll <- vector(length=(kk-1))
#   for (i in c(1:(kk-1))){sumvec_ll[i] <- sum(diag(AUC_3d[-(1:i),])/2) + sum(diag(AUC_3d[-(1:(i+1)),]))/2}
#   distvec <- c(C_ll, rev(sumvec_ll), M_ll,  M_ur, sumvec_ur, C_ur)
#
#   CD <- vector()
#   for (i in c(1:length(distvec))) {CD[i] <- sum(distvec[c(1:i)])}
#   xi_lo  <- findInterval(0.025,CD)
#   xi_up  <- findInterval(0.975, CD)
#
#   CIlo_I <- I_est - 4*sqrt(Var_I)/no_int*(length(distvec)/2-xi_lo)
#   CIup_I <- I_est + 4*sqrt(Var_I)/no_int*(xi_up+1-length(distvec)/2)
#   output<-data.frame("CIlo_Idiag"=CIlo_I, "CIup_Idiag"=CIup_I)
#
#   return (output)}
