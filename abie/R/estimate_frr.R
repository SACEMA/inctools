...
#' @export
frr_binomial <- function(data=data,
                         subid_var="sid",
                         time_var="time",
  ...
  data <- process_data(data=data, subid_var=subid_var, time_var=time_var, recency_vars=recency_vars, inclusion_time_threshold=1000000)
  ...
  for (subjectid in unique(data$sid)) {
    if (sum(data$recency_status[data$sid==subjectid]==1)/nrow(data[data$sid==subjectid,]) == 0.5)
  ...
  binomprob <- binom.test(ceiling(sum(subjectdata$recent)), nrow(subjectdata), p = 0,
                          conf.level = 1-alpha)
  FRR <- data.frame(round(binomprob$estimate[[1]],4),
                    round(binomprob$conf.int[1],4),
                    round(binomprob$conf.int[2],4),
                    alpha,
                    binomprob$statistic,
                    binomprob$parameter[[1]],
                    nrow(data))
