  ...
  fluidRow(
    ...
    column(3,
           wellPanel(
             ...
             selectInput(
               "minimal_precision",
               label = "Specify minimal precision of:",
               c(
                 "No minimal precision" = "none",
                 "Incidence difference" = "delta",
                 "Baseline incidence" = "baseline",
                 "Cohort incidence" = "cohort"
               ),
               selected = "none"
             ),
             conditionalPanel(
               condition = "input.minimal_precision == 'delta'",
               sliderInput("RSEdelta_required_perc","Maximum RSE on Incidence difference:", 
                           value = 25, step = 1, max = 150, min = 1) #,
               #sliderInput("FracIncRed","Incidence reduction (%):",min = 5, max = 95, value = 50, step = 5)
             ),
             conditionalPanel(
               condition = "input.minimal_precision == 'baseline'",
               span("Not implemented yet!", style="color:red")
             ),
             conditionalPanel(
               condition = "input.minimal_precision == 'cohort'",
               span("Not implemented yet!", style="color:red")
             )
    ...
    column(6,
           tabsetPanel(id = "tabs", type = "tabs",
                       tabPanel("Incidence reduction", value='tab1_val', id = 'tab1',
                                conditionalPanel(
                                  condition = "input.minimal_precision == 'none'",
                                    wellPanel(
                                ...
                                conditionalPanel(
                                  condition = "input.minimal_precision == 'delta'",
                                  wellPanel(
                                    h3("Sample size for power and precision"),
                                    p(textOutput("ss_power_text")),
                                    p(textOutput("ss_precision_text")),
                                    p(),
                                    h5(textOutput("deltaI_ci_header")),
                                    p(textOutput("deltaI_ci_text"))
                                  )
                                )
