...
```
Output is presented in list form, so individual tables can be called by the user. For example, if the returned function object is named `desired.power`, then the implied subject counts deduced from assumed prevalence and given sample size can be accessed with `desired.power$Implied.Subject.Counts` or `desired.power[[5]]`.
